...
export default function MobileListingControls() {
  ...
  return (
    <div className="lg:hidden">
      {/* Filter & Sort — fixed bottom navigation bar */}
      <div className="fixed bottom-4 inset-x-0 z-40 flex justify-center px-4">
        <div className="grid grid-cols-2 w-full max-w-xs bg-[#B8860B] text-[#FFF8E7] rounded-full shadow-lg overflow-hidden">
          <button
            onClick={() => setFilterOpen(true)}
            className="flex items-center justify-center gap-2 py-3 text-sm font-semibold border-r border-[#FFF8E7]/25 active:bg-[#9c7209]"
          >
            <SlidersHorizontal className="h-4 w-4" /> Filter
          </button>
          <button
            onClick={() => setSortOpen(true)}
            className="flex items-center justify-center gap-2 py-3 text-sm font-semibold active:bg-[#9c7209]"
          >
