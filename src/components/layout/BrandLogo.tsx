...
export default function BrandLogo({
  ...
  return (
    <span className="flex items-center gap-1">
      ...
      <span className={`relative flex-shrink-0 overflow-hidden rounded-lg ${compact ? 'h-10 w-10 md:h-12 md:w-12' : 'h-14 w-14 md:h-[5.75rem] md:w-[5.75rem]'} ${hoverZoom ? 'hover:z-[60]' : ''}`}>
        <Image
          src={logo}
      ...
      <span className="leading-tight flex flex-col items-stretch w-fit">
        <span
          className={`block font-bold tracking-wide ${compact ? 'text-sm md:text-lg' : 'text-base md:text-2xl'} ${light ? 'text-[#F4E5C2]' : 'text-[#4E1E24]'}`}
          style={{ fontFamily: 'var(--font-cormorant-upright), serif' }}
        >
          Dyuthi Pattu Sarees
        </span>
        <span className={`flex w-full items-center justify-center gap-1.5 md:gap-2 tracking-wide ${compact ? 'text-[8px] md:text-[10px]' : 'text-[11px] md:text-xs'} ${light ? 'text-[#C9A86A]' : 'text-[#A88C57]'}`}>
