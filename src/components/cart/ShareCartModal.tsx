...
const TEMPLATE = (link: string) =>
  `Hi,\n\nCheck out this collection I put together from Dyuthi Pattu Sarees.\n\nView it here:\n${link}\n\nPlease help me choose \u{1F60A}`
...
export default function ShareCartModal({
  ...
  useEffect(() => {
    ...
    fetch('/api/cart/share', {
      ...
      .then((data) => {
        if (cancelled) return
        if (data.code) setLink(`${window.location.origin}/cart/share/${data.code}`)
  ...
  const primaryShare = async () => {
    ...
    if (canNativeShare) {
      try {
        await navigator.share({ title: 'My Dyuthi Pattu Sarees collection', text: message })
      } catch {
  ...
  const targets: { label: string; icon: React.ReactNode; href?: string; onClick?: () => void; bg: string; show: boolean }[] = [
    { label: 'WhatsApp', icon: <WhatsAppIcon className="h-6 w-6" />, href: `https://wa.me/?text=${encMsg}`, bg: 'bg-[#25D366]', show: true },
    { label: 'WhatsApp Business', icon: <WhatsAppIcon className="h-6 w-6" />, href: `https://api.whatsapp.com/send?text=${encMsg}`, bg: 'bg-[#128C7E]', show: true },
    { label: 'Facebook', icon: <FacebookIcon className="h-6 w-6" />, href: `https://www.facebook.com/sharer/sharer.php?u=${encLink}`, bg: 'bg-[#1877F2]', show: true },
    { label: 'Telegram', icon: <Send className="h-6 w-6" />, href: `https://t.me/share/url?url=${encLink}&text=${encodeURIComponent(TEMPLATE('').trim())}`, bg: 'bg-[#0088cc]', show: true },
    { label: 'Email', icon: <Mail className="h-6 w-6" />, href: `mailto:?subject=${encodeURIComponent('My Dyuthi Pattu Sarees collection')}&body=${encMsg}`, bg: 'bg-[#AD1457]', show: true },
    { label: 'Instagram', icon: <InstagramIcon className="h-6 w-6" />, onClick: shareToInstagram, bg: 'bg-gradient-to-tr from-[#F58529] via-[#DD2A7B] to-[#8134AF]', show: true },
    { label: 'Twitter', icon: <TwitterIcon className="h-5 w-5" />, href: `https://twitter.com/intent/tweet?text=${encMsg}`, bg: 'bg-black', show: true },
    { label: 'Pinterest', icon: <PinterestIcon className="h-6 w-6" />, href: `https://www.pinterest.com/pin/create/button/?url=${encLink}&description=${encodeURIComponent(TEMPLATE('').trim())}`, bg: 'bg-[#E60023]', show: true },
    { label: 'Message', icon: <MessageSquare className="h-6 w-6" />, href: `sms:?&body=${encMsg}`, bg: 'bg-[#5BC236]', show: isMobile },
  ]

  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4" role="dialog" aria-label="Share collection">
      <div className="absolute inset-0 bg-black/50" onClick={onClose} />
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <h2 className="font-semibold text-[#4E1E24] text-lg">You are sharing a collection</h2>
